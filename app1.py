"""
app1.py — Streamlit Experimental Prototype
Sistem Rekomendasi Karir, Kursus & Visualisasi Nilai Capaian OBE Mahasiswa:
Rekomendasi Pembelajaran dari Knowledge Graph & Relative Skill Gap Analysis + Nilai CLO

Multi-Kampus: Telkom University Jakarta, Telkom University Surabaya, ITS Surabaya
Dataset: 300 Mahasiswa Terpadu | 496 Posisi Karir | 8.207 Kursus Online Master | 19.890 Nilai CLO
...
# ============================================================
st.set_page_config(
    page_title="OBE Career, Course & Student Grades Recommender",
    page_icon="🎓",
    layout="wide",
...
INPUT_KG_DIR = OUTPUT_DIR / "input_kg"
DATA_CLEAN_DIR = BASE_DIR / "data_clean" / "normalized"
...
def load_data():
    ...
    student_score_path = INPUT_KG_DIR / "student_skill_final.csv"
    if not student_score_path.exists():
        student_score_path = DATA_CLEAN_DIR / "student_skill_final_v2.csv"

    if not ranking_path.exists() or not course_path.exists():
        return None, None, None, f"File data tidak ditemukan di {OUTPUT_DIR}"

    df_rank = pd.read_csv(ranking_path, sep=';')
    df_course = pd.read_csv(course_path, sep=';')

    df_scores = None
    if student_score_path.exists():
        df_scores = pd.read_csv(student_score_path, sep=';')
    ...
    return df_rank, df_course, df_scores, None


df_rank, df_course, df_scores, load_error = load_data()
...
with st.sidebar:
    ...
    st.caption("Eksperimen: Nilai Mahasiswa + Karir & Kursus")
    st.divider()
    page = st.radio(
        "Navigation Menu:",
        [
            "🏠 Rekomendasi Karir & Kursus",
            "📊 Nilai Capaian Mahasiswa (OBE)"
        ]
    )
    ...
    st.divider()
    st.caption("📁 Dataset Aktif:\n- 300 Mahasiswa (ITS, Jakarta, Surabaya)\n- 496 Karir Industri\n- 8.207 Kursus Online Master\n- 19.890 Nilai CLO/Skill Mahasiswa")
...
# ==============================================================
# PAGE 1: REKOMENDASI KARIR & KURSUS
# ==============================================================
if page == "🏠 Rekomendasi Karir & Kursus":
    ...
    # Student scores data
    stu_score_df = pd.DataFrame()
    if df_scores is not None and not df_scores.empty:
        stu_score_df = df_scores[df_scores['student_id'] == selected_sid].copy()

    # ── Student Summary Metrics ───────────────────────────────
    col1, col2, col3, col4, col5 = st.columns(5)
    col1.metric("👤 Nama Mahasiswa", selected_name)
    col2.metric("🆔 Student ID", selected_sid)
    col3.metric("🏫 Asal Kampus", selected_campus)
    col4.metric("🎯 Top Karir", len(top5))
    if not stu_score_df.empty:
        avg_sc = stu_score_df['score'].mean()
        col5.metric("📈 Rata-rata Nilai OBE", f"{avg_sc:.1f} / 100")
    else:
        col5.metric("📈 Rata-rata Nilai", "N/A")
    st.divider()

    # ── Nilai Capaian Mahasiswa (Collapsible Section) ──────────
    if not stu_score_df.empty:
        with st.expander("📊 Lihat Detail Nilai Capaian CLO / Skill Mahasiswa Ini", expanded=False):
            st.markdown(f"#### 📝 Transkrip Capaian OBE — {selected_name} ({selected_sid})")
            
            sc_m1, sc_m2, sc_m3, sc_m4 = st.columns(4)
            sc_m1.metric("Total CLO / Skill Diambil", len(stu_score_df))
            sc_m2.metric("Rata-rata Nilai", f"{stu_score_df['score'].mean():.2f}")
            sc_m3.metric("Nilai Tertinggi", f"{stu_score_df['score'].max():.1f}")
            sc_m4.metric("Nilai Terendah", f"{stu_score_df['score'].min():.1f}")

            # Bar chart score per skill
            top_skills_chart = stu_score_df.sort_values('score', ascending=False).head(15)
            fig_bar = px.bar(
                top_skills_chart,
                x='score',
                y='canonical_skill',
                ...
                title=f'Top 15 Nilai Skill Capaian Tertinggi ({selected_name})',
                labels={'score': 'Nilai Capaian (0-100)', 'canonical_skill': 'Skill Canonical'},
                template='plotly_dark'
            )
            fig_bar.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(15,23,42,0.4)', yaxis={'categoryorder': 'total ascending'})
            st.plotly_chart(fig_bar, use_container_width=True)

            # Table
            display_tbl = stu_score_df[['id_CLO', 'skill_original', 'canonical_skill', 'score']].copy()
            display_tbl.columns = ['ID CLO', 'Skill Silabus (RPS)', 'Skill Canonical (Industri)', 'Nilai Capaian']
            display_tbl['Status Capaian'] = display_tbl['Nilai Capaian'].apply(lambda x: '✅ Lulus / Kompeten' if x >= 50.01 else '⚠️ Perlu Perbaikan')
            st.dataframe(display_tbl.sort_values('Nilai Capaian', ascending=False), use_container_width=True, hide_index=True)
            st.divider()
    ...
    sel_career_disp     = st.selectbox("Pilih Karir untuk Detail", career_options_list, label_visibility="visible")
    ...
    with tab2:
        if matched_skills:
            ...
            st.info("Belum ada skill yang cocok.")
    ...
    with tab3:
        if missing_skills:
            ...
            st.success("Semua skill sudah terpenuhi.")
    ...
    if not missing_skills:
        ...
        total_recs = 0
        for skill in missing_skills:
            courses = courses_map.get(skill, [])
            total_recs += len(courses)

            fb_info = ""
            if courses and '(via:' in courses[0].get('covers_label', ''):
                parent_tag = courses[0]['covers_label'].split('(via:')[1].replace(')', '').strip()
                fb_info = f' &nbsp;<span style="color:#F59E0B; font-size:0.75rem; font-style:italic;">[Fallback Domain: {parent_tag}]</span>'

            st.markdown(
                f'<div style="margin: 14px 0 6px 0; font-weight:600; font-size:0.95rem;">'
                f'Missing Skill: <span class="pill-red">{skill}</span>{fb_info}'
                f'</div>',
                unsafe_allow_html=True
            )

            if not courses:
                st.caption("ℹ️ Belum ada kursus spesifik yang sesuai di katalog.")
                continue

            for idx, c in enumerate(courses, 1):
                lvl_lbl = c.get('level_label', get_level_label(c.get('level', '')))
                dot_col = level_dot_colors.get(lvl_lbl, '#9CA3AF')

                with st.container(border=True):
                    c_title = c.get('course_name', 'Unknown Course').title()
                    plat_name = c.get('platform', 'Unknown')
                    
                    st.markdown(f"**#{idx} — 💻 {c_title}**")
                    st.markdown(
                        f'<div style="color: #94A3B8; font-size: 0.82rem; margin-top: 2px;">'
                        f'🏛️ {plat_name} &nbsp;|&nbsp; '
                        f'<span style="color: {dot_col};">●</span> Level: {lvl_lbl}'
                        f'</div>',
                        unsafe_allow_html=True
                    )
...
# ==============================================================
# PAGE 2: NILAI CAPAIAN MAHASISWA (OBE TRANSKRIP & KOMPARASI)
# ==============================================================
elif page == "📊 Nilai Capaian Mahasiswa (OBE)":
    st.markdown("## 📊 Nilai Capaian Akademik Mahasiswa (OBE Assessment)")
    st.caption("Transkrip Capaian Pembelajaran Lulusan (CPL / CLO) dan Nilai Penguasaan Skill Mahasiswa")
    st.divider()

    if df_scores is None or df_scores.empty:
        st.warning("Data nilai mahasiswa tidak ditemukan.")
    else:
        # Global metrics across all 300 students
        gm1, gm2, gm3, gm4 = st.columns(4)
        gm1.metric("👤 Total Mahasiswa", df_scores['student_id'].nunique())
        gm2.metric("📈 Rata-rata Nilai Keseluruhan", f"{df_scores['score'].mean():.2f} / 100")
        gm3.metric("📝 Total Entri Nilai CLO", f"{len(df_scores):,}")
        gm4.metric("🔧 Canonical Skills Terpetakan", df_scores['canonical_skill'].nunique())
        st.divider()

        # Campus comparison
        st.subheader("🏫 Perbandingan Capaian Nilai Rata-rata per Kampus")
        campus_stats = df_scores.groupby('campus')['score'].agg(['mean', 'median', 'std', 'count']).reset_index()
        campus_stats.columns = ['Kampus', 'Rata-rata Nilai', 'Median', 'Standar Deviasi', 'Jumlah Penilaian CLO']
        
        c_col1, c_col2 = st.columns([1, 1])
        with c_col1:
            st.dataframe(campus_stats.style.format({
                'Rata-rata Nilai': '{:.2f}',
                'Median': '{:.2f}',
                'Standar Deviasi': '{:.2f}',
                'Jumlah Penilaian CLO': '{:,}'
            }), use_container_width=True, hide_index=True)
            
        with c_col2:
            fig_camp = px.box(
                df_scores, x='campus', y='score', color='campus',
                labels={'score': 'Nilai Capaian (0-100)', 'campus': 'Kampus'},
                title='Distribusi Nilai Capaian per Kampus',
                template='plotly_dark'
            )
            fig_camp.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(15,23,42,0.4)')
            st.plotly_chart(fig_camp, use_container_width=True)

        st.divider()
        st.subheader("🔍 Cari & Analisis Transkrip Mahasiswa")
        
        c_f1, c_f2 = st.columns([1, 3])
        with c_f1:
            cmp_sel = st.selectbox("Pilih Kampus", ["Semua", "Jakarta", "Surabaya", "ITS"], key="score_campus")
        
        stu_list = df_scores[['student_id', 'student_name', 'campus']].drop_duplicates()
        if cmp_sel != "Semua":
            stu_list = stu_list[stu_list['campus'] == cmp_sel]
        
        stu_list['display'] = stu_list['student_id'] + " — " + stu_list['student_name'] + " (" + stu_list['campus'] + ")"
        
        with c_f2:
            sel_s = st.selectbox("Pilih Mahasiswa untuk Lihat Transkrip", stu_list['display'].tolist(), key="score_stu_sel")
            
        sid = sel_s.split(" — ")[0].strip()
        single_df = df_scores[df_scores['student_id'] == sid].copy()
        sname = single_df['student_name'].iloc[0]
        scamp = single_df['campus'].iloc[0]

        # Student Card
        with st.container(border=True):
            st.markdown(f"### 🎓 Transkrip OBE: {sname} ({sid}) — {scamp}")
            tm1, tm2, tm3, tm4 = st.columns(4)
            tm1.metric("Total CLO Lulus", len(single_df))
            tm2.metric("Rata-rata Nilai", f"{single_df['score'].mean():.2f}")
            tm3.metric("Nilai Maksimum", f"{single_df['score'].max():.1f}")
            tm4.metric("Nilai Minimum", f"{single_df['score'].min():.1f}")

        # Top skills distribution
        col_g1, col_g2 = st.columns([1, 1])
        with col_g1:
            fig_hist = px.histogram(
                single_df, x='score', nbins=15,
                title=f'Distribusi Nilai CLO ({sname})',
                labels={'score': 'Nilai Capaian'},
                color_discrete_sequence=['#10B981'],
                template='plotly_dark'
            )
            fig_hist.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(15,23,42,0.4)')
            st.plotly_chart(fig_hist, use_container_width=True)

        with col_g2:
            top_ten = single_df.sort_values('score', ascending=False).head(10)
            fig_top = px.bar(
                top_ten, x='score', y='canonical_skill', orientation='h',
                title='10 Skill Capaian Tertinggi',
                color='score', color_continuous_scale='Viridis',
                labels={'score': 'Nilai', 'canonical_skill': 'Skill'},
                template='plotly_dark'
            )
            fig_top.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(15,23,42,0.4)', yaxis={'categoryorder': 'total ascending'})
            st.plotly_chart(fig_top, use_container_width=True)

        # Full table
        st.markdown("#### 📋 Tabel Lengkap Nilai per Capaian Pembelajaran (CLO)")
        tbl_show = single_df[['id_CLO', 'skill_original', 'canonical_skill', 'score']].copy()
        tbl_show.columns = ['ID CLO', 'Skill Kurikulum (RPS)', 'Canonical Skill', 'Nilai (0-100)']
        tbl_show['Status'] = tbl_show['Nilai (0-100)'].apply(lambda v: '✅ Memenuhi Syarat (Lulus)' if v >= 50.01 else '⚠️ Di Bawah Standar')
        st.dataframe(tbl_show.sort_values('Nilai (0-100)', ascending=False), use_container_width=True, hide_index=True)
...
# ==============================================================
    ...
    with col_rel:
        with st.container(border=True):
            ...
            st.markdown("""
- **HAS_SKILL**: Skill yang dikuasai mahasiswa (berdasarkan nilai CLO ≥ 50.01)
